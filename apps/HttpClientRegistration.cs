    ...
    extension(IServiceCollection services)
    {
            ...
            int attemptTimeoutSeconds = 10)
        {
            services.AddHttpClient(name, c =>
                ...
                .AddResilienceHandler($"checker-{name}", (builder, context) =>
                {
                    // Resolved once per handler build; surfaces the real request behind each retry
                    // (the default resilience telemetry only logs an opaque "Operation Key: null").
                    var logger = context.ServiceProvider
                        .GetRequiredService<ILoggerFactory>()
                        .CreateLogger($"apps.Resilience.{name}");
                    ...
                    builder.AddRetry(new HttpRetryStrategyOptions
                    {
                        ...
                        OnRetry = args =>
                        {
                            ...
                            var outcome = response is { } r
                                ? $"{(int)r.StatusCode} {r.ReasonPhrase}"
                                : args.Outcome.Exception?.GetType().Name ?? "unknown failure";

                            logger.LogWarning(
                                args.Outcome.Exception,
                                "Retrying '{Client}' (attempt {Attempt}) in {DelayMs:F0}ms — {Method} {Uri} → {Outcome}; Retry-After: {RetryAfter}",
                                name,
