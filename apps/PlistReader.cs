...
public sealed class PlistReader(ILogger<PlistReader> logger)
{
    ...
    private static PlistInfo ParseXmlPlist(string appBundlePath, string xml)
    {
        ...
        var platformName = plist.GetString("DTPlatformName");
        var detectedPlatform = platformName switch
        {
            "iphoneos" => AppPlatform.IosOnMac, // Designed-for-iPad binary on Mac
            _ => AppPlatform.NativeMac, // "macosx" or absent => Mac (AppKit or Catalyst)
        };
        var isMacCatalystApp = plist.ContainsKey("UIDeviceFamily") && platformName == "macosx";
        var isIpadApp = platformName == "iphoneos";
        var isAppStoreApp = HasAppStoreReceipt(appBundlePath);
        var isWebApp = bundleId is not null &&
        ...
        var appType = AppAttribute.App;
        if (isMacCatalystApp)
        {
            appType |= AppAttribute.MacCatalystApp;
        }

        if (isWebApp)
        {
            appType |= AppAttribute.PwaApp;
        }

        if (isIpadApp || detectedPlatform == AppPlatform.IosOnMac)
        {
            appType |= AppAttribute.IosOrIpadApp;
        }

        if (sparkleUrl is not null)
        {
            appType |= AppAttribute.SparkleFeed;
        }

        if (isAppStoreApp)
        {
            appType |= AppAttribute.AppStoreApp;
        }

        if (detectedPlatform == AppPlatform.NativeMac)
        {
            appType |= AppAttribute.MacApp;
        }

        if (isElectronApp)
        {
            appType |= AppAttribute.ElectronApp;
        }

        if (isSafariExtension)
        {
            appType |= AppAttribute.SafariExtension;
        }

        // TODO: check if built by DevMate

        return new PlistInfo(displayName,
            shortVersion,
            bundleVersion,
            bundleId,
            sparkleUrl,
    ...
    public class Plist : IEnumerable<KeyValuePair<string, Plist>>
    {
        ...
        public List<Plist>? GetArray(string? key = null)
        {
            if (key is not null && _dictionary is not null)
            {
                return _dictionary.TryGetValue(key, out var value) ? value.GetArray() : null;
            }

            return IsArray ? _array : throw new InvalidOperationException("Plist value is not an array");
        }

        public Dictionary<string, Plist>? GetDictionary(string? key = null)
        {
            if (key is not null && _dictionary is not null)
            {
                return _dictionary.TryGetValue(key, out var value) ? value.GetDictionary() : null;
            }

            return IsDictionary ? _dictionary : throw new InvalidOperationException("Plist value is not a dictionary");
        }
