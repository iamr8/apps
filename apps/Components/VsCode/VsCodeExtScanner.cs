    ...
    : IScanner
{
    ...
    {
        return ScannerHelper.IsExecutableAvailable("code");
    }

    /// <inheritdoc/>
    public async IAsyncEnumerable<DiscoveredApp> ScanAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        var code = ScannerHelper.FindExecutable("code") ?? "code";
        var result = await runner.RunAsync(code, "--list-extensions --show-versions", cancellationToken);
        if (!result.Success)
        {
    ...
    /// </summary>
    private static DiscoveredApp? ParseLine(string line, Dictionary<string, string> displayNames)
    {
