...
public sealed class AppRecord(DiscoveredApp app)
{
    ...
    {
        if (this.App.UpdateMethod is not null)
        {
            return this.App.UpdateMethod;
        }

        if (this.SubApps is not null)
        {
            foreach (var subApp in this.SubApps)
            {
                var method = subApp.GetUpdateMethod();
